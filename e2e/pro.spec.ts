...
test('Scribe is unlocked in the pro build and flags a misspelling as you type', async () => {
  await nav('Scribe')
  await expect(page.getByText('Off Grid Pro · Available now')).toHaveCount(0)
  // Dismiss the "Set up your local AI" onboarding nudge so it doesn't blur the pad
  // (Scribe's rule checks are model-free, so the squiggle works without a model).
  ...
  await pad.fill('please recieve the photosynthesis notes in order to test')
  const misspelled = page.locator('span', { hasText: 'recieve' }).first()
  await expect(misspelled).toBeVisible({ timeout: 5000 })
  await page.waitForTimeout(700)
  // No wavy underline span should wrap the valid word.
  const validWordUnderline = page.locator('span[style*="underline"]', { hasText: 'photosynthesis' })
  await expect(validWordUnderline).toHaveCount(0)
  // Enumerate exactly what is underlined, so we assert the real state (not pixels).
  const underlined = await page.locator('span[style*="underline"]').allInnerTexts()
  // recieve (spelling) + the wordy "in order to" (clarity) are expected; nothing else.
  expect(underlined).toContain('recieve')
  expect(underlined.some((t) => /in order to/.test(t))).toBe(true)
  expect(underlined).not.toContain('photosynthesis')
