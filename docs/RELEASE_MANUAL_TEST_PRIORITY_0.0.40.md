...
- Tier 0 (blockers, no automation): __ / 3
- Tier 1 (manual-only P0 boundaries): __ / 11
- Tier 2 (P0 native seam, automation can't reach it): __ / 12
- Tier 3 (P0/P1 real-seam, spot-check): __ / 20
- Tier 4 (proven + polish, lowest priority): __ / 8
...
- [ ] **14. `[P0][NONE]` Windows 0.0.40 — or explicitly cut it.** On clean Windows: installer
  ...
  locked; both journeys work. *If Windows is not shipping tonight, record that decision and skip.*
...
- [ ] **15. `[P0][LOGIC-ONLY]` Fresh profile is truly empty.** New disposable profile.
  ...
  *(Ledger #9 "Configure for me" onboarding is PROVEN; the empty-start assertion is real-seam — but
  confirm on device.)*
...
- [ ] **26. `[P0][LOGIC-ONLY]` Recover from forced quit + engine crash + low disk.** Force-quit during
  ...
  *(Ledger #145 cold relaunch is PROVEN; the engine-crash + low-disk variants are not.)*
...
- [ ] **28. `[P0][REAL-SEAM]` Gateway network boundary.** **Expected:** loopback HTTP succeeds; the
  Mac's non-loopback interface is rejected. *(Real TCP integration.)*
...
- [ ] **39. `[P0][REAL-SEAM]` Vault item round-trip (Pro).** Create/reopen/edit/copy/delete a login,
  ...
  only the chosen field; binary attachment intact after lock/unlock. *(Ledger #127 copy-to-clipboard
  PROVEN.)*
...
- [ ] **46. `[P1][REAL-SEAM]` Settings persistence + residency + storage totals.** Change Core + Pro
  ...
  required; storage totals reasonably aligned. *(Ledger #129/#130 residency PROVEN.)*
