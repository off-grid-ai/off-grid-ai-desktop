...
- [ ] **D8 WRONG-CONVERSATION** — Regenerate / drained-queue / background-tab send builds history from the ACTIVE tab's `messages`, not the target conversation → the model answers with another conversation's context. `MemoryChat.tsx:886`
...
- [ ] **D10 TRUNCATION** — `chatStream` ignores the caller's `maxTokens` (`this.maxTokens || maxTokens`) → streamed answers capped at the persisted value regardless of the request. `llm.ts:621`
...
- [ ] **D20 DATA-ORPHAN + BROKEN-PROMISE** — Deleting a project orphans all its chats + artifacts: `deleteProject` sweeps the dead `project_threads` backend but never `rag_conversations` (no FK/cascade); the confirm dialog promises "delete its chats" but they persist, badged to a phantom project, grounding against an empty project. `store.ts:239`, `database.ts:363`
