...
export function enqueue<T>(byConv: Record<string, T[]>, convId: string, item: T): Record<string, T[]> {
  return { ...byConv, [convId]: [...(byConv[convId] ?? []), item] };
}

/** Pop the next queued item for a conversation; returns it + the updated map. */
export function dequeue<T>(byConv: Record<string, T[]>, convId: string): { item: T | undefined; next: Record<string, T[]> } {
