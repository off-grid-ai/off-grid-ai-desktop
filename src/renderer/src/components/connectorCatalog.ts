...
export const CONNECTOR_CATALOG: CatalogEntry[] = [
  ...
  },
  {
    ...
    blurb: 'Events, availability, scheduling',
    category: 'Communication',
    color: '#4285F4',
    letter: 'C',
    transport: 'http',
  ...
  },
  {
    id: 'discord',
    name: 'Discord',
    blurb: 'Servers, channels, messages',
    category: 'Communication',
    color: '#5865F2',
    letter: 'D',
    transport: 'stdio',
    ...
    args: ['-y', 'discord-mcp'],
    auth: 'token',
    secrets: [{ key: 'DISCORD_TOKEN', label: 'Bot token' }],
  ...
  },
  {
    ...
    args: ['-y', 'whatsapp-mcp'],
    auth: 'token',
    secrets: [{ key: 'WHATSAPP_SESSION', label: 'Session / config' }],
    docsUrl: 'https://github.com/lharries/whatsapp-mcp',
    ready: false
  },
  ...
  },
  {
    ...
    args: ['-y', 'coda-mcp'],
    auth: 'token',
    secrets: [{ key: 'CODA_API_KEY', label: 'API key' }],
  ...
  },
  {
    ...
    args: ['-y', 'obsidian-mcp'],
    auth: 'token',
    secrets: [{ key: 'OBSIDIAN_VAULT_PATH', label: 'Vault path' }],
  ...
  },
  {
    ...
    auth: 'token',
    secrets: [{ key: 'CLICKUP_API_KEY', label: 'API token' }],
  ...
  },
  {
    id: 'shortcut',
    name: 'Shortcut',
    blurb: 'Stories, epics, iterations',
    category: 'Project & Issues',
    color: '#3A2EFF',
    letter: 'S',
    transport: 'stdio',
    ...
    args: ['-y', '@shortcut/mcp'],
    auth: 'token',
    secrets: [{ key: 'SHORTCUT_API_TOKEN', label: 'API token' }],
  ...
  },
  {
    ...
    auth: 'token',
    secrets: [{ key: 'GITLAB_PERSONAL_ACCESS_TOKEN', label: 'Access token' }],
  ...
  },
  {
    id: 'cloudflare',
    name: 'Cloudflare',
    blurb: 'Workers, DNS, observability',
    category: 'Engineering',
    color: '#F38020',
    letter: 'C',
    transport: 'http',
  ...
  },
  {
    id: 'stripe',
    name: 'Stripe',
    blurb: 'Payments, customers, invoices',
    category: 'Engineering',
    color: '#635BFF',
    letter: 'S',
    transport: 'http',
  ...
  },
  {
    id: 'pagerduty',
    name: 'PagerDuty',
    blurb: 'Incidents, on-call, alerts',
    category: 'Engineering',
    color: '#06AC38',
    letter: 'P',
    transport: 'http',
  ...
  },
  {
    ...
    auth: 'token',
    secrets: [
      { key: 'POSTGRES_CONNECTION_STRING', label: 'Connection string', placeholder: 'postgres://…' }
    ],
  ...
  // ---------- Design ----------
  {
    id: 'figma',
    name: 'Figma',
    blurb: 'Designs, dev mode, components',
    category: 'Design',
    color: '#F24E1E',
    letter: 'F',
    transport: 'http',
  ...
  },
  {
    ...
    auth: 'token',
    secrets: [{ key: 'AMPLITUDE_API_KEY', label: 'API key' }],
  ...
  },
  {
    ...
    args: ['-y', 'mixpanel-mcp'],
    auth: 'token',
    secrets: [{ key: 'MIXPANEL_SERVICE_ACCOUNT', label: 'Service account' }],
  ...
  },
  {
    id: 'intercom',
    name: 'Intercom',
    blurb: 'Conversations, users, help',
    category: 'Product & Analytics',
    color: '#1F8DED',
    letter: 'I',
    transport: 'http',
  ...
  },
  {
    ...
    auth: 'token',
    secrets: [{ key: 'AIRTABLE_API_KEY', label: 'Personal access token', placeholder: 'pat…' }],
  ...
  // ---------- Automation ----------
  {
    id: 'zapier',
    name: 'Zapier',
    blurb: '8,000+ apps via one connector',
    category: 'Automation',
    color: '#FF4F00',
    letter: 'Z',
    transport: 'http',
...
export const CONNECTOR_SETUP_HINTS: Record<string, string> = {
  slack:
    'Create a Slack app, add bot scopes (channels:read, chat:write, search:read), install it to your workspace, then copy the Bot User OAuth Token (xoxb-…) and your Team ID (T…).',
  discord:
    'Discord Developer Portal → New Application → Bot → Reset/Copy Token. Enable the Message Content intent.',
  whatsapp:
    'Runs a local WhatsApp bridge. On first connect it shows a QR — scan it in WhatsApp → Linked Devices. Paste the session/config path it gives you.',
  ...
  github:
    'GitHub → Settings → Developer settings → Personal access tokens → generate one (repo + read:org scopes).',
  ...
  airtable:
    'Airtable → Builder Hub → Personal access tokens → create a token (pat…) with the scopes/bases you need.'
}
