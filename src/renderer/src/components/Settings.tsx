...
import { getRegisteredSettingsSections } from '@renderer/bootstrap/sectionRegistry'
  ...
  return (
    <div className="flex items-start justify-between gap-4">
      <p className="text-neutral-500 text-sm">
        Off Grid reaches out on its own - a morning briefing of your day and a heads-up ~20 min
        before each meeting with who is in it and your open items. Delivered as native
  ...
  return (
    <div>
      <p className="text-neutral-500 text-sm mb-4">
        Preferences distilled from the reasons you give when you dismiss a suggestion. This is the
        only learned text fed back to the assistant - it refreshes about once an hour, and raw notes
        are never used directly. You can remove any line you disagree with.
      </p>
      {lines.length ? (
        <ul className="divide-y divide-neutral-800/60 overflow-hidden rounded-xl border border-neutral-700/50 bg-neutral-800/40">
          {lines.map((line, i) => (
            <li key={i} className="group flex items-start gap-2 px-3 py-2">
              <span className="min-w-0 flex-1 whitespace-pre-wrap break-words font-mono text-sm leading-relaxed text-neutral-300">
                {line}
              </span>
              <button
        ...
        <p className="rounded-xl border border-neutral-700/50 bg-neutral-800/40 p-3 text-sm text-neutral-600">
          Nothing learned yet. When you dismiss a suggestion, tell Off Grid why - it generalizes the
          useful ones here.
        </p>
      )}
      {lines.length > 0 && (
        <div className="mt-3">
          <button
            onClick={clear}
            className="rounded-lg px-3 py-1.5 text-xs text-neutral-500 hover:text-white"
          >
            Clear all
          </button>
        </div>
...
function ProPlanSection(): React.ReactElement {
  ...
  const statusLine =
    info?.tier === 'lifetime'
      ...
      : info?.tier === 'monthly'
        ? `Monthly · active until ${fmt(info.expiry)}`
        : 'Pro active'
        ...
        <>
          ...
          <div className="mt-5">
            <div className="text-[11px] uppercase tracking-wide text-neutral-500">
              Devices ({devices.length} of {MAX_DEVICES})
            </div>
          ...
          {info?.tier === 'monthly' && (
            <div className="mt-5">
              ...
              <p className="mt-1 flex items-start gap-2 text-[11px] text-neutral-500">
                ...
                renewal email - one is sent with every payment.
              </p>
            </div>
          )}
        </>
...
export function Settings(): React.ReactElement {
  ...
  useEffect(() => {
    if (!isPro) return // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ;(window.api as any)
      .idGet?.()
      .then((id: { name: string; email: string }) => {
        if (id) {
          setIdName(id.name || '')
          setIdEmail(id.email || '')
        }
      ...
      <div className="relative flex-1 overflow-y-auto px-1 pt-5 pb-16">
        ...
        >
          ...
          {isPro ? (
            <SettingsCard
              title="You"
              summary="Who you are, so Off Grid can attribute your messages and calendar."
              delay={0.15}
            >
              <p className="text-neutral-500 text-sm mb-4">
                Tells Off Grid who you are - so it can tell your messages and commitments apart from
                everyone else&apos;s. Used to attribute action items and to make sense of your email
                and calendar.
              </p>
            ...
            <ProPlaceholder
              delay={0.15}
              title="You"
              description="Tell Off Grid who you are so it can attribute your messages, commitments, and calendar - part of the Pro intelligence layer."
            />
          )}
          ...
          {isPro ? (
            <SettingsCard
              ...
              summary="A morning briefing and a heads-up before each meeting."
              delay={0.18}
            >
              <ProactiveSection />
            </SettingsCard>
            ...
            <ProPlaceholder
              title="Proactive delivery"
              description="A morning briefing and a heads-up before each meeting - native notifications, even when the window is closed."
            />
          )}
          {isPro ? (
            <SettingsCard
              title="What Off Grid has learned"
              summary="Preferences distilled from your dismissals, fed back to the assistant."
              delay={0.22}
            >
              <SecretaryPrefs />
            </SettingsCard>
            ...
            <ProPlaceholder
              title="What Off Grid has learned"
              description="Preferences distilled from the suggestions you dismiss, fed back to your assistant so it gets sharper over time."
            />
          )}
          {isPro && (
            <SettingsCard
              title="Your Pro plan"
              summary="Your subscription, devices, and how to cancel."
              delay={0.3}
            >
              <ProPlanSection />
            </SettingsCard>
          ...
          {/* Registered pro Settings sections (e.g. Scribe writing companion). Core
              renders whatever pro registered via the section-registry seam; free builds
              register nothing, so this is empty. No pro logic lives here. */}
          {getRegisteredSettingsSections().map((section) => {
