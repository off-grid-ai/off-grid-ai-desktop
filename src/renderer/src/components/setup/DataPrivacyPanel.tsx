...
export function DataPrivacyPanel(): React.ReactElement {
  ...
  const refresh = useCallback(async () => {
    try { const c = await api?.getDataSummary?.(); if (Array.isArray(c)) setCats(c as DataCategory[]); }
    catch { /* keep last */ }
  }, [api]);
  ...
  const RETENTION: Record<string, { label: string; days: number }[]> = {
    captures: [{ label: '> 3 days', days: 3 }, { label: '> 7 days', days: 7 }, { label: '> 30 days', days: 30 }],
    meetings: [{ label: '> 7 days', days: 7 }, { label: '> 30 days', days: 30 }, { label: '> 90 days', days: 90 }],
  };
  ...
  const clearOne = async (c: DataCategory, olderThanDays?: number): Promise<void> => {
    ...
    if (!window.confirm(`Delete ${what}? This permanently removes it from this device and can't be undone.`)) return;
    setBusy(c.id);
    try { await api?.clearDataCategory?.(c.id, olderThanDays); await refresh(); } finally { setBusy(null); }
  };

  const deleteEverything = async (): Promise<void> => {
    if (!window.confirm('Delete ALL your data — chats, memory, captures, meetings, and generated images? This cannot be undone. (Installed models are kept.)')) return;
    if (!window.confirm('Are you absolutely sure? This permanently erases your personal data on this device.')) return;
    setBusy('all');
    try {
    ...
    <div className="rounded-xl border border-neutral-800 bg-neutral-900/40 font-mono">
      ...
      <div className="divide-y divide-neutral-800/40">
        {cats.map((c) => {
          ...
          const meta = [c.count != null ? `${c.count.toLocaleString()} item${c.count === 1 ? '' : 's'}` : null, size].filter(Boolean).join(' · ');
          return (
            <div key={c.id} className="flex items-center gap-3 px-4 py-2.5">
              <div className="min-w-0 flex-1">
                <div className="text-xs text-neutral-200">{c.label}</div>
                <div className="text-[11px] text-neutral-500">{c.detail}{meta ? ` — ${meta}` : ''}</div>
              </div>
              <div className="flex shrink-0 items-center gap-1.5">
                ...
                >
                  <Trash className="h-3 w-3" /> {busy === c.id ? 'Clearing…' : RETENTION[c.id] ? 'All' : 'Clear'}
                </button>
              </div>
            </div>
          );
        })}
        {cats.length === 0 && <div className="px-4 py-6 text-center text-xs text-neutral-600">Reading…</div>}
      </div>
