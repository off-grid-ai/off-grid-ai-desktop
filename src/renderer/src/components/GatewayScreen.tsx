...
const ENDPOINTS: { label: string; method: string; path: string; note: string }[] = [
  {
    label: 'Chat (text + vision)',
    method: 'POST',
    path: '/v1/chat/completions',
    note: 'OpenAI-compatible; image_url parts for vision'
  },
  {
    label: 'Text → Image',
    method: 'POST',
  ...
  },
  {
    ...
    note: '{ input, voice? } → audio/wav'
  },
  {
    label: 'Embeddings',
    method: 'POST',
    path: '/v1/embeddings',
    note: 'local all-MiniLM-L6-v2 · 384-dim'
  },
      ...
      <div className="flex w-full flex-col gap-6 px-8 py-1 pb-16">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-neutral-500">
              One local, OpenAI-compatible API for every model you download — text, vision, image,
              voice. Runs on your device; nothing leaves it.
            </p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-neutral-800 bg-neutral-900/60 p-5">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              onClick={() => open('/docs')}
              className="flex items-center gap-1.5 rounded-lg border border-neutral-700 px-3 py-2 text-xs text-neutral-300 hover:border-green-500/60 hover:text-white"
            >
              <IconExternalLink className="h-4 w-4" /> Interactive playground
            </button>
          </div>
          <p className="mt-3 text-xs leading-relaxed text-neutral-500">
            Point any OpenAI client here — no API key needed. Run it headless with{' '}
            <code className="text-neutral-300">--server-only</code> to deploy just the gateway.
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-neutral-800 bg-neutral-900/60 p-5">
          ...
          <pre className="overflow-x-auto rounded-lg border border-neutral-800 bg-neutral-950 p-4 text-xs leading-relaxed text-neutral-200">
