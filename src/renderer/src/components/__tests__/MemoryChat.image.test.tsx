...
type GenPayload = { steps?: number; model?: string; width?: number; height?: number; prompt?: string };
...
function installApi(opts: { active: string; models: string[]; settings?: Record<string, unknown> }) {
  ...
  const api = {
    ...
    imageGenStatus: vi.fn(async () => ({ available: true, models: opts.models, active: opts.active })),
    ...
    saveSetting: vi.fn(async (k: string, v: unknown) => { settings[k] = v; }),
...
describe('<MemoryChat/> image mode — the generateImage payload is the terminal artifact', () => {
  ...
  it('carries the USER-typed steps (10), not the model default (28), and the picked model', async () => {
    ...
    const { generateImage, setActiveModalModel } = installApi({ active: FULL, models: [FULL, FEW_STEP] });
    renderChat();

    await openImageComposer(user);
    // Model select must be present (imgModels.length > 1) and reflect the active model.
    const modelSelect = (await screen.findByDisplayValue(/dreamlike-photoreal-v2/i, {}, { timeout: 3000 })) as HTMLSelectElement;
