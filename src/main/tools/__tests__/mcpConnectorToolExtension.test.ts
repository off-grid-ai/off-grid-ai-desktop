...
vi.mock('../../mcp', () => ({
  ...
  setConnectorStatus: (id: number, status: string, detail?: string | null) =>
    setConnectorStatus(id, status, detail)
}))
...
describe('isActionTool', () => {
  it.each([
    ...
    'whoami_now',
    'describe_table'
  ])('treats read-verb tool %s as a non-action (no approval)', (tool) => {
  ...
  describe('schemas', () => {
    ...
    it('skips a connector whose fetchTools rejects but still returns others', async () => {
      ...
      fetchTools
        ...
        .mockResolvedValueOnce([{ name: 'read_it' }])
  ...
  describe('execute', () => {
    ...
    it('routes a write tool through the approval hook and does NOT call the connector when queued', async () => {
      ...
      expect(callHook).toHaveBeenCalledWith(
        'mcp:proposeApproval',
        expect.objectContaining({ tool: 'send_message', connector: 'Slack' })
      )
