...
describe('recommendedImageModelId', () => {
  ...
  it('keys off the Light tag, not the model name', () => {
    const genericFull: ModelEntry = {
      id: 'x/base-GGUF',
      name: 'Base',
      kind: 'image',
      tags: ['Fast'],
      files: [{ name: 'base-Q8.gguf', url: '' }]
    }
    const genericLight: ModelEntry = {
      id: 'x/base-GGUF-Q4',
      name: 'Base Light',
      kind: 'image',
      tags: ['Fast', 'Light'],
      files: [{ name: 'base-Q4.gguf', url: '' }]
    }
  ...
  it('prefers the Versatile all-rounder when several Light models exist (order-independent)', () => {
    ...
    const otherLight: ModelEntry = {
      id: 'x/photo-GGUF-Q4',
      name: 'Photo Light',
      kind: 'image',
      tags: ['Photoreal', 'Light'],
      files: [{ name: 'photo-Q4.gguf', url: '' }]
    }
    const otherFull: ModelEntry = {
      id: 'x/photo-GGUF',
      name: 'Photo',
      kind: 'image',
      tags: ['Photoreal'],
      files: [{ name: 'photo-Q8.gguf', url: '' }]
    }
  ...
  it('lists a Light variant for every offgrid image model in the catalog', () => {
    const imageFamilies = CATALOG.filter(
      (m) => m.kind === 'image' && m.id.startsWith('offgrid-ai/') && !/-Q4$/.test(m.id)
    )
    for (const fam of imageFamilies) {
      const hasLight = CATALOG.some(
        (m) => m.id === `${fam.id}-Q4` && (m.tags ?? []).includes('Light')
      )
