...
describe('computeSafeCtx — the freeze fix', () => {
  it('clamps the 8B-on-16GB case to the value we shipped (21504, not 64K)', () => {
    ...
    const ctx = computeSafeCtx({ requested: 65536, totalGb: MAC_16, weightsGb: 6.2, kvType: 'f16', frac: 0.65, reserveGb: 1.5 });
    expect(ctx).toBe(21504);
    expect(ctx).toBeLessThan(65536);
  });

  it('never exceeds the requested context', () => {
    const ctx = computeSafeCtx({ requested: 8192, totalGb: 64, weightsGb: 5, kvType: 'f16', frac: 0.65, reserveGb: 1.5 });
    expect(ctx).toBeLessThanOrEqual(8192);
  });

  it('never drops below the 2048 floor even on a tiny/over-subscribed machine', () => {
    const ctx = computeSafeCtx({ requested: 65536, totalGb: 8, weightsGb: 7.5, kvType: 'f16', frac: 0.45, reserveGb: 2.0 });
  ...
  it('allows a LARGER context when the KV cache is quantized', () => {
    const base = { requested: 65536, totalGb: MAC_16, weightsGb: 6.2, frac: 0.65, reserveGb: 1.5 } as const;
  ...
  it('Conservative mode yields a smaller context than Extreme', () => {
    ...
    const small = computeSafeCtx({ requested: 65536, totalGb: MAC_16, weightsGb: 6.2, kvType: 'f16', ...cons });
    const big = computeSafeCtx({ requested: 65536, totalGb: MAC_16, weightsGb: 6.2, kvType: 'f16', ...ext });
...
describe('chooseChatModel — the "don\'t pick the biggest" fix', () => {
  const CATALOG: SizingModel[] = [
    { id: 'qwen-2b', kind: 'vision', params: 2, minRamGb: 4, files: [{ sizeBytes: 1.1 * GB }, { sizeBytes: 0.8 * GB }] },
    { id: 'gemma-e4b', kind: 'vision', params: 4, minRamGb: 6, files: [{ sizeBytes: 4.98 * GB }, { sizeBytes: 0.99 * GB }] }, // ~5.97GB
    { id: 'qwen-8b', kind: 'vision', params: 8, minRamGb: 8, files: [{ sizeBytes: 5.0 * GB }, { sizeBytes: 1.66 * GB }] }, // ~6.66GB
  ...
  it('Conservative mode (tighter budget) drops to a smaller model', () => {
    const pick = chooseChatModel(CATALOG, 16, 30, 0.30); // budget 4.8GB → only the 2B fits comfortably
  ...
  it('falls back to the smallest text/vision model when nothing fits comfortably', () => {
    const tiny = chooseChatModel(CATALOG, 4, 30, 0.10); // 0.4GB budget — nothing comfy
    expect(tiny).not.toBeNull();
    expect(['text', 'vision']).toContain(tiny?.kind);
  });

  it('returns null when the catalog has no text/vision models', () => {
    expect(chooseChatModel([{ id: 'x', kind: 'image', files: [{ sizeBytes: GB }] }], 16, 30, 0.5)).toBeNull();
  });
});
...
describe('recommendedParamCeiling — 16GB must default to a 4B', () => {
  ...
  it('with the ceiling applied, a 16/17GB Mac picks the 4B even when the 8B fits by bytes', () => {
    ...
    const catalog: SizingModel[] = [
      { id: 'gemma-e4b', kind: 'vision', params: 4, minRamGb: 6, files: [{ sizeBytes: 5.97 * GB }] },
...
describe('fitLevel — RAM-fit badge thresholds', () => {
  ...
  it('totalBytes sums all files (primary + mmproj)', () => {
    expect(totalBytes({ kind: 'vision', files: [{ sizeBytes: 2 * GB }, { sizeBytes: 1 * GB }] })).toBe(3 * GB);
  });
});
