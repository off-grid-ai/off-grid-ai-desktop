...
vi.mock('@xenova/transformers', () => ({ pipeline: async () => { throw new Error('no embedding model in test'); }, env: {} }));
...
vi.mock('@lancedb/lancedb', () => ({ connect: async () => ({ openTable: async () => ({}), tableNames: async () => [] }) }));
...
beforeAll(async () => {
  ...
  `);
  try { db.exec('ALTER TABLE entities ADD COLUMN hidden INTEGER NOT NULL DEFAULT 0'); } catch { /* already present */ }
});
beforeEach(() => {
  fake.reset();
...
function seedObservation(summary: string, surface: string): string {
  const id = Number(getDB().prepare('INSERT INTO observations (summary, surface, category) VALUES (?, ?, ?)').run(summary, surface, 'work').lastInsertRowid);
  return `obs:${id}`;
}
...
describe('search_memory citations — real universalSearch over the real full schema', () => {
  it('surfaces a real keyword hit as a structured citation in r.unified', async () => {
    ...
    expect(cite, `expected a citation for ${key} in ${JSON.stringify(r.unified.map((s) => s.key))}`).toBeTruthy();
