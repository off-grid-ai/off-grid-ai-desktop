...
class FakeEngine implements ManagedRuntime {
  ...
  constructor(modality: Modality) { this.modality = modality; }
  evict(): void { this.loaded = false; }        // free memory
  warm(): void { this.loaded = true; }          // resident re-warm: reload now
  release(): void { /* on-demand: stay down, lazy-load on next use */ }
}
...
function store(initial: Partial<Record<Modality, ResidencyMode>>) {
  ...
  return {
    ...
    set: (m: Modality, mode: ResidencyMode) => { map[m] = mode; },
  };
}
...
describe('residency integration (real queue + real seam + real engines)', () => {
  ...
  it('two heavy engines never run at once; the queued one waits', async () => {
    ...
    const first = q.run({ tier: 2, label: 'image-a' }, async () => { order.push('a-start'); await gate; order.push('a-end'); });
    const second = q.run({ tier: 2, label: 'image-b' }, async () => { order.push('b-start'); });
