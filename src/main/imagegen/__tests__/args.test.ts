...
describe('buildCoreMLArgs', () => {
  it('builds the ANE helper flag vector with the default 16 steps', () => {
    const args = buildCoreMLArgs({ model: '/m/coreml', prompt: 'a cat', outPath: '/o.png', seed: 42 });
  ...
  it('honours an explicit step count and adds --negative only when non-empty', () => {
    const args = buildCoreMLArgs({ model: 'm', prompt: 'p', outPath: 'o', seed: 1, steps: 8, negativePrompt: '  ugly  ' });
  ...
  it('omits --negative for a whitespace-only negative prompt (edge)', () => {
    const args = buildCoreMLArgs({ model: 'm', prompt: 'p', outPath: 'o', seed: 1, negativePrompt: '   ' });
    expect(args).not.toContain('--negative');
  });
});
...
describe('buildZImageArgs', () => {
  const base = {
    model: '/m/zimage.gguf', llm: '/m/qwen.gguf', vae: '/m/ae.safetensors',
    prompt: 'a fox', outPath: '/o.png', seed: 7, threads: '6',
...
describe('buildStandardArgs', () => {
  const common = {
    prompt: 'a landscape', outPath: '/o.png', seed: 5, threads: '4',
    previewArgs: ['--preview', 'proj'], modelFlags: ['-m', '/m/model.gguf'],
  };
  ...
  it('passes model flags through verbatim (UNET-only diffusion + companions)', () => {
    const modelFlags = ['--diffusion-model', '/m/unet.gguf', '--clip_l', '/m/l', '--clip_g', '/m/g', '--vae', '/m/vae'];
  ...
  it('honours explicit width/height/steps/cfg overrides', () => {
    const args = buildStandardArgs({ ...common, base: 'sd-1.5.gguf', width: 640, height: 384, steps: 20, cfgScale: 5 });
  ...
  it('taesd path (fastVae) is preferred and suppresses VAE-tiling even on a large XL image', () => {
    const args = buildStandardArgs({ ...common, base: 'animagine-xl.gguf', width: 1024, height: 1024, taesdPath: '/m/taesdxl.safetensors' });
  ...
  it('adds VAE-tiling only for an XL model whose largest side exceeds 768', () => {
    ...
    const at768 = buildStandardArgs({ ...common, base: 'animagine-xl.gguf', width: 768, height: 768 });
    expect(at768).not.toContain('--vae-tiling');
    // XL at 1024 -> tiling
    const at1024 = buildStandardArgs({ ...common, base: 'animagine-xl.gguf', width: 1024, height: 768 });
  ...
  it('adds img2img flags with the default strength, and a custom strength when given', () => {
    ...
    const a2 = buildStandardArgs({ ...common, base: 'sd-1.5.gguf', initImage: '/in.png', strength: 0.4 });
