...
describe('buildInferenceFields', () => {
  ...
  it('omits language for auto (server auto-detects) and includes a real language', () => {
    expect(
      buildInferenceFields({ wavPath: '/tmp/a.wav', language: 'auto' }).language
    ).toBeUndefined()
  ...
  it('includes and clamps the prompt, dropping an empty one', () => {
    ...
    expect(
      buildInferenceFields({ wavPath: '/tmp/a.wav', prompt: 'Acme Corp, Kubernetes' }).prompt
    ).toBe('Acme Corp, Kubernetes')
