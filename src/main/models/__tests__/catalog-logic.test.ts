...
const local: LocalModelLike = { id: 'local:my.gguf', name: 'my', primary: 'my.gguf', kind: 'text', sizeBytes: 100 };
const localVision: LocalModelLike = { id: 'local:v.gguf', name: 'v', primary: 'v.gguf', mmproj: 'v-mmproj.gguf', kind: 'vision', sizeBytes: 200 };
const dl: DownloadedModelLike = { id: 'org/hf', name: 'HF', kind: 'vision', files: ['hf.gguf', 'hf-mmproj.gguf'] };
const catEntry: CatalogEntry = { id: 'cat/text', name: 'Cat Text', kind: 'text', files: [{ name: 'cat.gguf', role: 'primary' }] };
const catMflux: CatalogEntry = { id: 'cat/mflux', name: 'Flux', kind: 'image', runtime: 'mflux', files: [] };

const presentAll = (): boolean => true;
const presentNone = (): boolean => false;
const presentOnly = (names: string[]) => (n: string): boolean => names.includes(n);
...
describe('localsForCatalog', () => {
  it('includes locals whose primary file is present, tagged Imported, org Local', () => {
    ...
    expect(out).toEqual([{
      id: 'local:my.gguf', name: 'my', kind: 'text', org: 'Local', params: undefined,
      tags: ['Imported'], files: [{ name: 'my.gguf', url: '', sizeBytes: 100 }],
    }]);
  });
...
describe('downloadedForCatalog', () => {
  it('includes only fully-installed downloads, tagged Downloaded, org Hugging Face', () => {
    ...
    expect(out).toEqual([{
      id: 'org/hf', name: 'HF', kind: 'vision', org: 'Hugging Face',
      tags: ['Downloaded'], files: [{ name: 'hf.gguf', url: '' }, { name: 'hf-mmproj.gguf', url: '' }],
    }]);
  });
...
describe('mergeCatalog — order + all three sources', () => {
  it('emits locals, then installed downloads, then the catalog, in that order', () => {
    const out = mergeCatalog({
      locals: [local], downloaded: [dl], installedDownloadedIds: ['org/hf'],
  ...
  it('empty locals + empty downloads => just the catalog', () => {
    const out = mergeCatalog({ locals: [], downloaded: [], installedDownloadedIds: [], catalog: [catEntry], present: presentAll });
    expect(out).toEqual([catEntry]);
  });
  it('a not-installed download is omitted even if in the registry', () => {
    const out = mergeCatalog({ locals: [], downloaded: [dl], installedDownloadedIds: [], catalog: [], present: presentAll });
    expect(out).toEqual([]);
  });
});
...
describe('installedIds — order + per-source predicate', () => {
  it('locals, then downloaded, then present catalog ids', () => {
    const out = installedIds({
      locals: [local], installedDownloadedIds: ['org/hf'], catalog: [catEntry],
      present: presentAll, mfluxCached: () => false,
    });
    expect(out).toEqual(['local:my.gguf', 'org/hf', 'cat/text']);
  });
  it('drops a local whose file is gone; keeps a present catalog id', () => {
    const out = installedIds({
      locals: [local], installedDownloadedIds: [], catalog: [catEntry],
      present: presentOnly(['cat.gguf']), mfluxCached: () => false,
    });
    expect(out).toEqual(['cat/text']);
  });
  it('includes an mflux id only when cached', () => {
    const both = installedIds({ locals: [], installedDownloadedIds: [], catalog: [catMflux], present: presentNone, mfluxCached: () => true });
    const none = installedIds({ locals: [], installedDownloadedIds: [], catalog: [catMflux], present: presentAll, mfluxCached: () => false });
...
describe('primaryFileName', () => {
  it('prefers the role:primary file', () => {
    expect(primaryFileName({ files: [{ name: 'a.gguf' }, { name: 'p.gguf', role: 'primary' }] })).toBe('p.gguf');
  });
...
describe('buildDiskEntry — source resolution + active flag', () => {
  const sizeOf = (name: string): number => ({ 'my.gguf': 100, 'v.gguf': 200, 'v-mmproj.gguf': 50, 'hf.gguf': 300, 'hf-mmproj.gguf': 60, 'cat.gguf': 400 } as Record<string, number>)[name] ?? 0;
  ...
  it('imported local: sums primary + mmproj, kind local, active when it is the chat id', () => {
    const e = buildDiskEntry({
      id: 'local:v.gguf', locals: [localVision], downloaded: [], catalogById: () => undefined,
      isCatalogId: () => false, activeChatId: 'local:v.gguf', modals: noModals, sizeOf,
    });
  ...
  it('imported local: not active when it is not the chat id', () => {
    const e = buildDiskEntry({
      id: 'local:my.gguf', locals: [local], downloaded: [], catalogById: () => undefined,
      isCatalogId: () => false, activeChatId: null, modals: noModals, sizeOf,
    });
  ...
  it('free-form download: reads kind/files from the registry, active by modality filename match', () => {
    const e = buildDiskEntry({
      id: 'org/hf', locals: [], downloaded: [dl], catalogById: () => undefined,
      isCatalogId: () => false, activeChatId: null,
      modals: { image: null, speech: null, transcription: null }, sizeOf,
    });
  ...
  it('download id that IS also a catalog id falls through to the catalog branch', () => {
    const e = buildDiskEntry({
      id: 'org/hf', locals: [], downloaded: [dl], catalogById: (id) => (id === 'org/hf' ? { ...catEntry, id: 'org/hf', name: 'FromCatalog' } : undefined),
      isCatalogId: (id) => id === 'org/hf', activeChatId: null, modals: noModals, sizeOf,
    });
    expect(e.name).toBe('FromCatalog');
  });
  it('catalog model: sums catalog files, name/kind from the entry', () => {
    const e = buildDiskEntry({
      id: 'cat/text', locals: [], downloaded: [], catalogById: (id) => (id === 'cat/text' ? catEntry : undefined),
      isCatalogId: (id) => id === 'cat/text', activeChatId: 'cat/text', modals: noModals, sizeOf,
    });
  ...
  it('unknown id (no source): name falls back to the id, bytes 0, inactive', () => {
    const e = buildDiskEntry({
      id: 'ghost', locals: [], downloaded: [], catalogById: () => undefined,
      isCatalogId: () => false, activeChatId: null, modals: noModals, sizeOf,
    });
  ...
  it('image catalog model is active when a modality pick matches its primary filename', () => {
    const img: CatalogEntry = { id: 'cat/img', name: 'Img', kind: 'image', files: [{ name: 'img.safetensors', role: 'primary' }] };
    const e = buildDiskEntry({
      id: 'cat/img', locals: [], downloaded: [], catalogById: () => img, isCatalogId: () => true,
      activeChatId: null, modals: { image: 'img.safetensors', speech: null, transcription: null },
      sizeOf: () => 0,
    });
...
describe('protectedNames — orphan protection', () => {
  it('unions catalog files, local names, downloaded names, and active primary+mmproj', () => {
    const known = protectedNames({
      catalog: [catEntry], localNames: ['my.gguf'], downloadedNames: ['hf.gguf', 'hf-mmproj.gguf'],
      activePrimary: 'active.gguf', activeMmproj: 'active-mmproj.gguf',
    });
    expect([...known].sort()).toEqual(['active-mmproj.gguf', 'active.gguf', 'cat.gguf', 'hf-mmproj.gguf', 'hf.gguf', 'my.gguf']);
  });
  it('ignores null/absent active selection', () => {
    const known = protectedNames({ catalog: [], localNames: [], downloadedNames: [], activePrimary: null, activeMmproj: null });
...
describe('scanModelDir — total + orphans', () => {
  ...
  it('an unknown .part is an orphan (matched by its bare name)', () => {
    const out = scanModelDir({
      entries: ['half.gguf.part'], known: new Set(['other.gguf']),
