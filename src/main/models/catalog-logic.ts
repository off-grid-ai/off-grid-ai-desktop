...
export interface DownloadedModelLike { id: string; name: string; kind: string; files: string[] }
...
export function buildDiskEntry(opts: {
  ...
  if (lm) {
    ...
    return {
      ...
      kind: 'local',
      bytes,
      active: isModelActive({ kind: 'local', id, activeChatId: opts.activeChatId, modals: opts.modals }),
    };
  }
  ...
  if (dl && !opts.isCatalogId(id)) {
    ...
    return {
      ...
      kind: dl.kind,
      bytes,
      active: isModelActive({ kind: dl.kind, id, primaryFile: primary, activeChatId: opts.activeChatId, modals: opts.modals }),
    };
  }
  ...
  return {
    ...
    kind: e?.kind,
    bytes,
    active: isModelActive({ kind: e?.kind, id, primaryFile: primary, activeChatId: opts.activeChatId, modals: opts.modals }),
  };
}
