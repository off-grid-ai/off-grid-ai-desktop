...
export function downloadMfluxModel(
  modelId: string,
  ...
  return new Promise<void>((resolve, reject) => {
    const child = spawn(
      ...
      [
        '-c',
        `from huggingface_hub import snapshot_download; snapshot_download(${JSON.stringify(def.hfRepo)})`
      ],
    ...
    child.on('close', (code) => {
      if (code === 0 && isMfluxModelCached(modelId)) {
        onProgress?.(100)
        resolve()
...
function buildMfluxArgs(params: MfluxGenParams, outPath: string): string[] {
  ...
  const args = [
    ...
    outPath,
    '--steps',
    String(params.steps ?? def.defaultSteps),
    '--width',
    String(params.width ?? def.defaultSize),
    '--height',
    String(params.height ?? def.defaultSize)
  ]
...
function parseMfluxProgress(s: string): { step: number; total: number; secPerStep: number } | null {
  ...
  if (m)
    return { step: parseInt(m[1]!, 10), total: parseInt(m[2]!, 10), secPerStep: parseFloat(m[3]!) }
  return null
}
