...
describe('errMeta', () => {
  it('defaults to 500 / server_error for an error with no status', () => {
    expect(errMeta(new Error('boom'))).toEqual({ status: 500, type: 'server_error', message: 'boom' });
  });
  ...
  it('stringifies a non-Error thrown value', () => {
    expect(errMeta('plain string')).toEqual({ status: 500, type: 'server_error', message: 'plain string' });
  });
