...
describe('postCompletionOnce over real sockets (the ECONNRESET fix)', () => {
  ...
  it('REPRODUCES the bug: a pooled keep-alive agent DOES reset on reuse (fails-before)', async () => {
    ...
    const once = (): Promise<void> => new Promise((resolve, reject) => {
      const req = http.request({ hostname: '127.0.0.1', port, path: '/v1/chat/completions', method: 'POST', agent }, (res) => {
    ...
    try { await once(); } catch (e) { reused = e as Error; }
    agent.destroy();
    // Either it reset, or (timing-dependent) it recovered; if it reset, confirm it's the exact class.
    if (reused) { expect(reused.message).toMatch(/ECONNRESET|socket hang up|EPIPE/i); }
