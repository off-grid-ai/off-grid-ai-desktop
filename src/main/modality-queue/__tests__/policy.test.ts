...
describe('selectNext', () => {
  ...
  it('tier-3 yields while a tier-1 or tier-2 job is RUNNING', () => {
    ...
    expect(selectNext([{ id: 'r', tier: 2, label: 'r', seq: 1 }], waiting, DEFAULT_POLICY)).toBeNull();
    expect(selectNext([{ id: 'r', tier: 1, label: 'r', seq: 1 }], waiting, DEFAULT_POLICY)).toBeNull();
  });
